...
class hourglass(nn.Module):
    ...
    def forward(self, x):
        ...
        # print(f'self.conv5(conv4) size = {self.conv5(conv4).shape} self.redir(conv2) shape = {self.redir2(conv2).shape}')
        conv5 = F.relu(self.conv5(conv4) + self.redir2(conv2), inplace=True)

        # print(f'self.conv6(conv5) size = {self.conv6(conv5).shape} self.redir1(x) shape = {self.redir1(x).shape} x shape = {x.shape}')
        conv6 = F.relu(self.conv6(conv5) + self.redir1(x), inplace=True)
        return conv6

class ACV_Chan_Rest_Net_b2h4(nn.Module):
    def __init__(self, maxdisp, attn_weights_only, freeze_attn_weights):
        super(ACV_Chan_Rest_Net_b2h4, self).__init__()
        ...
        self.rest_module = Restormer(inp_channels=320,dim=320,num_blocks=[2],heads=[4],ffn_expansion_factor=1.66)
    ...
    def forward(self, left, right):
        ...
        if self.freeze_attn_weights:
            ...
            features_left["gwc_feature"] = self.rest_module(features_left["gwc_feature"])
            features_right["gwc_feature"] = self.rest_module(features_right["gwc_feature"])
            ...
            # print(f'patch_volume shape = {patch_volume.shape}')
            cost_attention = self.dres1_att_(patch_volume)
            # print(f'cost_attention shape = {cost_attention.shape}')
            cost_attention = self.dres2_att_(cost_attention)
            att_weights = self.classif_att_(cost_attention)
        ...
        if self.training:
            ...
            if not self.freeze_attn_weights:
                ...
                # Modified by gsr
                pred_attention = disparity_regression(pred_attention, self.maxdisp).unsqueeze(1)
            ...
            if not self.attn_weights_only:
                ...
                # Modified by gsr
                pred0 = disparity_regression(pred0, self.maxdisp).unsqueeze(1)
                ...
                # Modified by gsr
                pred1 = disparity_regression(pred1, self.maxdisp).unsqueeze(1)
                ...
                # Modified by gsr
                pred2 = disparity_regression(pred2, self.maxdisp).unsqueeze(1)
