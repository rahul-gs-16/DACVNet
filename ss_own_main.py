...
from models import model_loss_train_ss, model_loss_test_ss,apply_disparity
from models import model_loss_train_ss_apply_disparity,model_loss_test_ss_apply_disparity
from utils import *
...
def train():
    ...
    for epoch_idx in range(start_epoch, args.epochs):
        ...
        for batch_idx, sample in enumerate(TrainImgLoader):
            ...
            i_loss,d_loss, loss, scalar_outputs, image_outputs = train_sample(sample,compute_metrics=do_summary)
            if do_summary:
            ...
            if (batch_idx + 1)%100 == 0:
                print('Epoch {}/{}, Iter {}/{}, i_loss = {:.3f}, d_loss = {:.3f}, train loss = {:.3f}, time = {:.3f}'.format(epoch_idx, args.epochs,
                                                                                                batch_idx,len(TrainImgLoader),i_loss,d_loss, loss,
        ...
        for batch_idx, sample in enumerate(TestImgLoader):    
            ...
            del scalar_outputs, image_outputs
            if (batch_idx + 1)%100 == 0:
                print('Epoch {}/{}, Iter {}/{}, test loss = {:.3f}, time = {:3f}'.format(epoch_idx, args.epochs,batch_idx,len(TestImgLoader), loss,
                                                                                        time.time() - start_time))
...
def train_sample(sample, compute_metrics=False):
    ...
    # print(f'warped_right min = ',warped_right.min(),' val at (0,0,128,255) = ',warped_right[0,0,128,255])
    loss = image_loss + disp_grad_loss
    scalar_outputs = {"image_loss": image_loss, "disp_grad_loss":disp_grad_loss, "loss":loss}
    image_outputs = {"disp_est": disp_ests, "imgL": imgL, "imgR": imgR,"warped_left":warped_left,"warped_right":warped_right}
    if False:
    # if compute_metrics:
        with torch.no_grad():
            ...
            scalar_outputs["EPE"] = [EPE_metric(disp_est.squeeze(1), disp_gt.squeeze(1), mask.squeeze(1)) for disp_est in disp_ests]
            scalar_outputs["D1"] = [D1_metric(disp_est.squeeze(1), disp_gt.squeeze(1), mask.squeeze(1)) for disp_est in disp_ests]
            scalar_outputs["Thres1"] = [Thres_metric(disp_est.squeeze(1), disp_gt.squeeze(1), mask.squeeze(1), 1.0) for disp_est in disp_ests]
            scalar_outputs["Thres2"] = [Thres_metric(disp_est.squeeze(1), disp_gt.squeeze(1), mask.squeeze(1), 2.0) for disp_est in disp_ests]
            scalar_outputs["Thres3"] = [Thres_metric(disp_est.squeeze(1), disp_gt.squeeze(1), mask.squeeze(1), 3.0) for disp_est in disp_ests]
    loss.backward()
    optimizer.step()
    return tensor2float(image_loss),tensor2float(disp_grad_loss),tensor2float(loss), tensor2float(scalar_outputs), image_outputs
    # return tensor2float(image_loss),0.,tensor2float(loss), scalar_outputs, image_outputs



# test one sample
@make_nograd_func
#(sample, test_left_syn,test_right_syn,compute_metrics=do_summary)
def test_sample(sample, compute_metrics=True):
    model.eval()
    imgL, imgR = sample['left'], sample['right']
    # imgL, imgR, disp_gt = sample['left'], sample['right'], sample['disparity']
    # print('******************** test_sample imgL shape = ',imgL.shape,' disp_gt shape = ',disp_gt.shape)
    imgL = imgL.cuda()
    imgR = imgR.cuda()
    # disp_gt = disp_gt.cuda()
    # mask = (disp_gt < args.maxdisp) & (disp_gt > 0)
    ...
    disp_ests[0] = disp_ests[0].unsqueeze(1)
    loss,warped_left,warped_right = model_loss_test_ss_apply_disparity(imgL,imgR,disp_ests)
    # warped_left = apply_disparity(imgR,-disp_ests[0],imgR.shape[-1]) # -disp_ests[0]
    # warped_right = apply_disparity(imgL,disp_ests[0],imgL.shape[-1])
